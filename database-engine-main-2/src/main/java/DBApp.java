...
public class DBApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        db.parseSQL(new StringBuffer("create table GutenMorgen ( id INT check (id>=1 and id<=1000),id2 INT check (id>=1 and id<=1000),name varchar check (name>=a and name<=zzzzzz),name2 varchar check (name>=a and name<=zzzzzz),name3 varchar check (name>=a and name<=zzzzzz),gpa DOUBLE check (gpa>=0.7 and gpa<=4.0),dob DATE check (dob>=1990/01/01 and dob<=2023/05/16), PRIMARY KEY(id))"));

        //db.parseSQL(new StringBuffer("create index indexName on GutenMorgen (id,dob,gpa)"));
        //db.parseSQL(new StringBuffer("create index indexName on GutenMorgen (name2,name2,name3,name2)"));

        for(int i=1;i<=20;i++) {
            double gpa = Math.random()*3 +1;
            String date = 1990+i + "/02/03";
            db.parseSQL(new StringBuffer("insert into GutenMorgen (id,id2,name,name2,name3,gpa,dob) values ("+i+","+i+",marwan,marwan,marwan,"+gpa+","+date+")"));
        }

        db.parseSQL(new StringBuffer("delete from GutenMorgen where id=2 and name=marwan"));

        db.parseSQL(new StringBuffer("update GutenMorgen set (dob=2002/06/08) where id=3"));

        Iterator i = db.parseSQL(new StringBuffer("select * from GutenMorgen where id2>=1 and name2 = marwan"));
